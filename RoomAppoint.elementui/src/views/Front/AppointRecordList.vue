                ...
                <el-row :gutter="10" class="EditFromBody">
                    ...
                    <el-col :span="24">
                        <el-form-item label="追评">
                            <el-input type="textarea" :rows="4" v-model="followUpComment" placeholder="请输入追评内容" :clearable="true"></el-input>
                            <div class="follow-up-actions">
                                <el-button type="primary" size="mini" @click="SubmitFollowUpComment">提交追评</el-button>
                            </div>
                        </el-form-item>
                    </el-col>
                </el-row>
    ...
    methods: {
        ...
        async ShowDetailModal(Id) {
            try {
                ...
                this.$message.error('获取评论详情失败：' + error.message);
            }
        },
        /**
         * 提交追评
        ...
        async SubmitFollowUpComment() {
            try {
                if (!this.followUpComment || this.followUpComment.trim() === '') {
                    this.$message.error('请输入追评内容');
                    return;
                }
                ...
                const replyData = {
                    ...
                    ReplyContent: this.followUpComment.trim(),
                    ReplyUserId: this.UserId // 当前登录的用户ID
                };
                
                // 发送保存请求
                const success = await this.$Post('/CommentReply/Save', replyData);
                
                if (success) {
                    this.$message.success('追评保存成功');
                    this.followUpComment = '';
                    
                    // 重新加载评论详情，显示最新追评
                    await this.ShowDetailModal(this.formData.Id);
                } else {
                    this.$message.error('追评保存失败');
                }
            } catch (error) {
                this.$message.error('追评保存失败：' + error.message);
            }
        },
        /**
         * 单个删除的时候会触发
