...
CREATE TABLE `CommentReply`  (
  ...
  `CreationTime` timestamp(0) NULL DEFAULT NULL COMMENT '创建时间',
  `CreatorId` int(11) NULL DEFAULT NULL COMMENT '创建人',
  `AppointRecordId` int(11) NULL DEFAULT NULL COMMENT '预约记录ID',
  `ReplyContent` varchar(255) CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci NULL DEFAULT NULL COMMENT '回复内容',
  `ReplyUserId` int(11) NULL DEFAULT NULL COMMENT '回复人ID',
  `ReplyTime` datetime(0) NULL DEFAULT NULL COMMENT '回复时间',
  PRIMARY KEY (`Id`) USING BTREE,
  INDEX `FK_CommentReply_AppointRecord`(`AppointRecordId`) USING BTREE,
  INDEX `FK_CommentReply_AppUser`(`ReplyUserId`) USING BTREE,
  CONSTRAINT `FK_CommentReply_AppointRecord` FOREIGN KEY (`AppointRecordId`) REFERENCES `appointrecord` (`Id`) ON DELETE CASCADE ON UPDATE CASCADE,
  CONSTRAINT `FK_CommentReply_AppUser` FOREIGN KEY (`ReplyUserId`) REFERENCES `appuser` (`Id`) ON DELETE CASCADE ON UPDATE CASCADE
